...
function pmd_ci_openjdk_install_adoptopenjdk() {
    ...
    local -r jdk_os=$(pmd_ci_get_os)

    if [ -z ${openjdk_version} ]; then
        log_error "OpenJDK Version is missing!"
        return 1;
    fi

    log_info "Installing Adopt OpenJDK Version ${openjdk_version} (${jdk_os})"
...
function pmd_ci_openjdk_install_zuluopenjdk() {
    ...
    log_info "Installing Zulu OpenJDK Version ${openjdk_version} (${jdk_os})"
    ...
    log_debug "zulu api: ${zulu_api_url}"
    local -r download_url=$(curl --silent -X GET "${zulu_api_url}" \
        -H "accept: application/json" \
        | jq -r ".url")
    if [[ -z "${download_url}" || "${download_url}" == "null" ]]; then
        log_error "No jdk found for download: ${zulu_api_url}"
        return 1
    fi
...
function pmd_ci_openjdk_setdefault() {
    ...
    log_info "Using OpenJDK ${openjdk_version} in ${target_dir} as default"

    if [ ! -e "${target_dir}/bin/java" ]; then
        log_error "Java executable ${target_dir}/bin/java not found!"
        return 1
    fi
